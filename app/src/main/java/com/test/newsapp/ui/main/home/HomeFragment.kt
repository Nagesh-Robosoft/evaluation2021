        ...
        viewModel.newsLiveData.observe(this, androidx.lifecycle.Observer {
            if (viewModel.isFirstPage()) {
                ...
                it.subList(1, it.size - 1).forEach {
                    tempList.add(Model(id = it.articleId.toString(), data = it, type = Model.TOP_NEWS_TYPE, text = ""))
                }
                list.clear()
                list.addAll(tempList)
            } else {
                it.forEach {
                    list.add(Model(id = it.articleId.toString(), data = it, type = Model.TOP_NEWS_TYPE, text = ""))
                }
            }
